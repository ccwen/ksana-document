...
var combineMarkups=function(db,markups,fn,pageid) {
...
var saveMarkup=function(opts,cb){
	var markups=combineMarkups(opts.db,opts.markups,opts.filename,opts.i);
...
var __loadMarkups=function(db,key,pagecount,cb) {
	var data=[];
...
var loadMarkup=function(db,key,pageid,cb) {
	if (pageid<0) {
		__loadMarkups(db,key,-pageid,cb);
		return;
	}
